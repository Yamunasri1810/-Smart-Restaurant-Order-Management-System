...
menu = [
    ...
    {"id": 5, "name": "Minestrone Soup", "price": 7},
]
...
def owner_data():
    ...
    if request.args.get("export") == "pdf":
        ...
        for row in export_rows:
            line = f"{row['Date']} | {row['Customer']} | {row['Dish']} | Qty: {row['Quantity']} | Price: ₹{row['Price']} | Total: ₹{row['Total']}"
            pdf.cell(200, 10, txt=line, ln=True)
        response = make_response(pdf.output(dest='S').encode('latin-1'))
        response.headers.set('Content-Disposition', 'attachment', filename='dashboard_export.pdf')
